...
Exposes 6 read-only tools over Databricks Jobs, Workspace, and Unity Catalog lineage data,
matching the contract the "databricks-job-lineage" skill expects (see that skill's
references/mcp_interface.md), plus two write-capable tools for opsbuddy-fix-style
real-verification (re-running a job to confirm a fix actually works):
...
mcp = MCPServer("databricks-lineage")
